...
class Solve_w_ECOS():
    ...
    def condentropy__orig(self,pdf,output):
        ...
        if output == 2: print("Time to compute H(T|XYZ) of the input pdf:", itoc - itic, "secs")
        return mysum
    #^ condentropy__orig()
    ...
    def entropy_V(self, V, pdf, output):
        ...
        if V == 1:
            ...
            #^ for
            itoc = time.process_time()
            if output == 2: print("Time to compute H(T):", itoc - itic, "secs")
            return mysum
...
def condent_V(V, p, output = 0):
    ...
    if V == 1:
        ...
        #^ for tx exists
        itoc = time.process_time()
        if output == 2: print("Time to compute H(T|X):", itoc - itic, "secs")
        return mysum
    #^ if
    ...
    if V == 2:
        ...
        #^ for ty exists
        itoc = time.process_time()
        if output ==2: print("Time to compute H(T|Y):", itoc - itic, "secs")
        return mysum
    #^ if
    ...
    if V == 3:
        ...
        #^ for tz exists
        itoc = time.process_time()
        if output == 2: print("Time to compute H(T|Z):", itoc - itic, "secs")
    return mysum
    #^ if
#^ condent_V()
    ...
    """Computes the partial information decomposition of  (T,X,Y,Z)
       ...
       Permission to use and modify under Apache License version 2.0
        ...
        Args:
             ...
             parallel: string - determines whether computation will be done
                                in parallel
